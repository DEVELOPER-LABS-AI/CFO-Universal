 ...
 *
 * Embeddable component for Mercury expense categorization within an accordion.
 * Tabs: "Uncategorized" (category = OTHER) | "All Expenses" (with re-categorize)
...
/** A Mercury expense record returned by the API. */
interface Expense {
  id: string;
  amount: number;
...
/** Available expense category options. */
...
/** Lookup map from category value to human-readable label. */
const CATEGORY_LABELS: Record<string, string> = Object.fromEntries(
  EXPENSE_CATEGORIES.map((c) => [c.value, c.label])
);

/**
 * Determines which association dropdowns a category requires.
 *
 * - CONTRACTOR_COST requires a contractor selection.
 * - SUBSCRIPTION / TOOLS optionally accept a client selection.
 * - All others need no additional association.
 */
...
/**
 * Renders expense categorization UI with Radix Tabs for
 * uncategorized / all-expenses views.
 *
 * @param props.organizationId - The current organization identifier.
 */
export function ExpensesSection({ organizationId }: ExpensesSectionProps) {
  ...
  // Re-categorize state (shared contractors/clients lists)
  ...
  /** Loads uncategorized expenses (category = OTHER) with contractor/client lists. */
  const loadExpenses = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await fetch(
        `/api/mercury/expenses/uncategorized?organizationId=${organizationId}`
      );
      if (!response.ok) {
  ...
  /** Loads all Mercury expenses (used by the "All Expenses" tab). */
  const loadAllExpenses = useCallback(async () => {
    try {
      setAllLoading(true);
      const response = await fetch(
        `/api/mercury/expenses/all?organizationId=${organizationId}`
      );
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || 'Failed to load expenses');
  ...
  }, [organizationId, contractors.length, clients.length]);

  // Load uncategorized expenses on mount.
  useEffect(() => {
    if (organizationId) loadExpenses();
  }, [organizationId, loadExpenses]);

  // ----------------------------------------------------------------
  // Handlers
  // ----------------------------------------------------------------

  /** Selects a category for an expense and resets contractor/client selections. */
  ...
  /**
   * Categorizes (or re-categorizes) an expense.
   *
   * @param expenseId - The expense to categorize.
   * @param isReCat - Whether this is a re-categorization from the "All" tab.
   */
  const handleCategorize = async (expenseId: string, isReCat = false) => {
    ...
    try {
      if (isReCat) setReSaving(expenseId);
      else setSaving(expenseId);
      ...
      if (isReCat) {
        // Update in-place in allExpenses.
        setAllExpenses((prev) =>
          prev.map((e) =>
            e.id === expenseId
              ? {
                  ...
                  contractor: contractorMap[expenseId]
                    ? {
                        id: contractorMap[expenseId],
                        name:
                          contractors.find((c) => c.id === contractorMap[expenseId])?.name ?? '',
                      }
                    : null,
                  client: clientMap[expenseId]
                    ? {
                        id: clientMap[expenseId],
                        name: clients.find((c) => c.id === clientMap[expenseId])?.name ?? '',
                      }
                    : null,
                }
              : e
        ...
        setReSelectedCategories((prev) => {
          const u = { ...prev };
          delete u[expenseId];
          return u;
        });
        setReSelectedContractors((prev) => {
          const u = { ...prev };
          delete u[expenseId];
          return u;
        });
        setReSelectedClients((prev) => {
          const u = { ...prev };
          delete u[expenseId];
          return u;
        });
        setRecatOpen(null);
        // Invalidate uncategorized cache.
        loadExpenses();
      } else {
        ...
        // Invalidate all expenses cache.
        setAllLoaded(false);
      }
    } catch (err: unknown) {
      toast.error(getErrorMessage(err) || 'Failed to categorize expense');
    } finally {
      if (isReCat) setReSaving(null);
      else setSaving(null);
    }
  };

  /** Lazily load all expenses when the user switches to the "all" tab. */
    ...
    <div>
      ...
      <Tabs defaultValue="uncategorized" onValueChange={handleTabChange}>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {expenses.map((expense) => {
                      ...
                      const associations = selectedCategory
                        ? getCategoryAssociations(selectedCategory)
                        : { needsContractor: false, needsClient: false };
                      return (
                        <tr key={expense.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <select
                              value={selectedCategories[expense.id] || ''}
                              onChange={(e) => handleSelectCategory(expense.id, e.target.value)}
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            {associations.needsContractor ? (
                              <select
                                value={selectedContractors[expense.id] || ''}
                                onChange={(e) =>
                                  setSelectedContractors((prev) => ({
                                    ...prev,
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            {associations.needsClient ? (
                              <select
                                value={selectedClients[expense.id] || ''}
                                onChange={(e) =>
                                  setSelectedClients((prev) => ({
                                    ...prev,
                                    [expense.id]: e.target.value,
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm">
                            <button
                              onClick={() => handleCategorize(expense.id, false)}
                              disabled={!selectedCategories[expense.id] || saving === expense.id}
        ...
        <TabsContent value="all">
          {allLoading ? (
            <Spinner text="Loading all expenses..." />
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {allExpenses.map((expense) => {
                      const linkedName =
                        ...
                        expense.client?.name ??
                        null;
                      const linkedType = expense.contractor
                        ? 'Contractor'
                        : expense.agency
                        ? 'Agency'
                        : expense.client
                        ...
                        <tr key={expense.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {linkedName ? (
                              <span>
                                {linkedName}{' '}
                                <span className="text-xs text-gray-400">({linkedType})</span>
                              </span>
                            ) : (
                            ...
                            {isOpen ? (
                              <div className="flex flex-col gap-2 min-w-[200px]">
                                <select
                                  ...
                                  className="block w-full px-2 py-1.5 border border-gray-300 rounded-md text-xs focus:ring-blue-500 focus:border-blue-500"
                                >
                                ...
                                {reAssoc?.needsContractor && (
                                  <select
                                    value={reSelectedContractors[expense.id] || ''}
                                    onChange={(e) =>
                                      setReSelectedContractors((prev) => ({
                                        ...prev,
                                        [expense.id]: e.target.value,
                                ...
                                {reAssoc?.needsClient && (
                                  <select
                                    value={reSelectedClients[expense.id] || ''}
                                    onChange={(e) =>
                                      setReSelectedClients((prev) => ({
                                        ...prev,
                                        [expense.id]: e.target.value,
                                ...
                                <div className="flex gap-2">
                                  <button
                                    onClick={() => handleCategorize(expense.id, true)}
                                    disabled={!reCategory || reSaving === expense.id}
                                    className="px-2 py-1 text-xs font-medium rounded text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
                                  >
                                  ...
                                  <button
                                    ...
                                    className="px-2 py-1 text-xs font-medium rounded text-gray-600 border border-gray-300 hover:bg-gray-50"
                                  >
                                    Cancel
                                  </button>
                                </div>
                              ...
                              <button
                                onClick={() => setRecatOpen(expense.id)}
